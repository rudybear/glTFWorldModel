...
### V8.1 -- remediation (2026-07-30)

**V8's acceptance text originally overclaimed that the transport-conversion was "verified at both 3-trial and full-150-trial scale"** -- in fact only 3 trials (the first 3 in sorted order) were ever run through the glTF-Validator; the "full-150" claim rested on all 150 trials converting without a Python exception, which is a different, weaker claim. An independent verifier caught this and found the real defect: `compute_vertex_normals` (`src/gltfworld/physion/convert.py`) produced zero-length NORMAL vectors whenever a vertex's area-weighted face-normal sum canceled to exactly zero, which glTF-Validator correctly flagged as `ACCESSOR_VECTOR3_NON_UNIT` on **12/150** real converted Collide GLBs. Fixed in V8.1 with a documented fallback (first adjacent face normal, then `+Y`, see `docs/VERIFICATION.md`'s V8 section), covered by two new synthetic-mesh unit tests, and the stage-2 test's sampling widened from `files[:3]` to 6 deterministically spread trials plus the pinned previously-failing trial. All 150 GLBs were regenerated from scratch and individually re-validated: **150/150, 0 errors, 0 warnings** -- this is now a genuinely observed result, not an inference. The OCP evaluation was re-run end-to-end against the regenerated GLBs and every accuracy number is bit-for-bit unchanged (GT-contact oracle 92.0% held-out / 92.67% full-150; our dynamics 49.0% held-out / 52.67% full-150; ballistic identical binary accuracy, 2.317m vs. 102.536m median divergence) -- expected, since vertex normals are a visual-only attribute and never feed the state-based OCP pipeline, but confirmed rather than assumed. Also corrected: the "merge + 4 stages" commit-count framing undercounted V8 by one -- it is actually **6** unpushed commits (V8-prep, the merge, three numbered stage commits, and the final V8 commit), not 5.
